// 繁體中文文案
export default {
  app: {
    name: 'Git 備份管理系統',
    brand: 'GIT·BACKUP',
    sub: 'ops console',
    online: 'SYSTEM ONLINE',
    repoTooltip: 'GitHub 倉庫 · {repo}'
  },
  layout: {
    skip: '跳到主要內容', navigation: '控制導覽', archiveNode: '歸檔節點',
    controlRoom: 'Archive Control Room', toggleTheme: '切換明暗主題'
  },
  platform: {
    label: '平臺',
    tab: '選擇備份平臺',
    github: 'GitHub',
    gitcode: 'GitCode',
    gitee: 'Gitee',
    disabledHint: '當前平臺尚未啟用，請勾選「啟用平臺」並儲存後再使用',
    hostLabel: '{platform} 倉庫', ready: '已啟用', offline: '未啟用'
  },
  nav: {
    dashboard: '儀表板',
    config: '備份設定',
    backup: '執行備份',
    jobs: '任務歷史',
    users: '用戶中心',
    schedule: '定時任務'
  },
  common: {
    save: '儲存',
    cancel: '取消',
    delete: '刪除',
    edit: '編輯',
    confirm: '確定',
    logout: '登出',
    login: '登入',
    username: '使用者名稱',
    password: '密碼',
    role: '角色',
    status: '狀態',
    actions: '操作',
    back: '返回',
    add: '新增',
    test: '測試',
    none: '無',
    untitled: '未命名',
    loading: '載入中…',
    enablePlatform: '啟用平臺'
  },
  status: {
    success: '成功',
    failed: '失敗',
    running: '進行中',
    cancelling: '取消中',
    cancelled: '已取消',
    unknown: '未知',
    idle: '閒置'
  },
  login: {
    prompt: '> auth.login --user',
    usernamePh: '使用者名稱',
    passwordPh: '密碼',
    submit: '登 入',
    hint: '預設帳號 admin / admin · 請儘快至「用戶中心」修改',
    error: '登入失敗', accessPoint: '受控歸檔入口 / 01', heroTitle: '把三處程式碼資產，收進同一座檔案庫。',
    heroDesc: '統一管理 GitHub、GitCode 與 Gitee 的備份設定、執行記錄和自動排程。', archiveReady: '歸檔通道',
    securityNote: '存取憑據僅用於受控的 HTTPS Git 工作階段。', credentials: '身分憑據', welcome: '進入控制室',
    formDesc: '驗證管理員身分後查看備份節點狀態。'
  },
  dashboard: {
    title: '設定概覽',
    platform: '當前平臺',
    repo: '倉庫',
    branch: '分支',
    sources: '備份來源數量',
    lastJob: '最近任務',
    overview: '設定概覽',
    gitUser: '{platform} 使用者',
    repoName: '倉庫名稱',
    branchL: '分支',
    backupDir: '備份目錄',
    serverId: '備份節點',
    autoIp: '自動偵測(IP)',
    admin: '管理員',
    sourcesL: '備份來源',
    notConfigured: '未設定',
    tokenWarn: '尚未設定 {platform} Token，請先至「備份設定」頁面填寫後再執行備份',
    distTitle: '任務狀態分布',
    totalJobs: '任務總數',
    recentTitle: '近期活動',
    noJobs: '暫無任務記錄', noJobsDesc: '執行一次備份後，任務軌跡會顯示在這裡。',
    commandCenter: '三平台歸檔控制台 / Live', heroTitle: '一屏掌握全部程式碼備份節點。',
    heroDesc: '同時查看三個平台的啟用狀態、倉庫目標、任務品質與最新執行軌跡。',
    runNow: '立即執行備份', reviewConfig: '檢查設定', archiveHealth: 'Archive health', healthGood: '運行健康',
    healthWatch: '需要關注', healthSetup: '等待設定', metrics: '歸檔關鍵指標', enabledPlatforms: '已啟用平台',
    enabledNote: '支援 GitHub / GitCode / Gitee', successRate: '任務成功率', completedJobs: '基於 {count} 個已完成任務',
    runningJobs: '正在運行', liveQueue: '即時任務佇列', latestHundred: '最近 100 筆記錄', platformMatrix: 'Platform matrix',
    threePlatformTitle: '三平台歸檔矩陣', managePlatforms: '管理平台', noRuns: '尚未執行', runBackup: '執行備份',
    activityIndex: 'Activity index', viewAll: '查看全部', jobQuality: 'Job quality',
    distributionSummary: '成功 {success}，失敗 {failed}，運行中 {running}，已取消 {cancelled}',
    credentialNote: 'Token 不會寫入遠端 URL 或任務日誌，Git 工作階段透過臨時憑據通道完成。'
  },
  config: {
    title: '備份設定',
    sectionRepo: '{platform} 倉庫',
    gitUser: '{platform} 使用者名稱 / 命名空間',
    gitToken: '{platform} Token',
    tokenPh: '輸入或修改 Token',
    tokenConfigured: 'Token 已設定，點擊眼睛可查看完整內容',
    tokenNotConfigured: '尚未設定 Token',
    showToken: '顯示 Token',
    hideToken: '隱藏 Token',
    tokenLoadError: '讀取 Token 失敗',
    repoName: '倉庫名稱',
    branch: '分支',
    sectionBackup: '備份設定',
    backupDir: '臨時任務目錄',
    backupDirHelp: '僅在任務期間存放 Git commit/tree 中繼資料與新生成的壓縮物件；不會檢出遠端檔案，也不會複製完整來源目錄，任務結束後會自動清理。',
    serverName: '備份節點名稱',
    serverNamePh: '例如：blog-prod',
    serverNameHelp: '作為遠端倉庫中的第一層目錄名稱，用於區分不同 RepoArk 實例；名稱會儲存在資料庫中，不依賴容器 ID。',
    nodeRequired: '請填寫備份節點名稱',
    sources: '備份來源路徑',
    sourcePh: '/path/to/file_or_dir',
    sourceRequired: '請至少新增一個備份來源路徑',
    addPath: '新增路徑',
    hostRoot: '宿主機根路徑映射',
    hostRootPh: 'Docker 部署時填 /host，留空則直接按來源路徑讀取',
    hostRootHelp: 'Docker 部署時，請把宿主機根目錄唯讀掛載到容器的 /host（如 -v /:/host:ro），此處填 /host 後，備份來源路徑無需加前綴，體驗與二進位直接運行一致。',
    save: '儲存設定',
    saved: '設定已儲存',
    saveError: '儲存失敗',
    enabled: '啟用該平臺'
  },
  backup: {
    title: '執行備份',
    tabHint: '每個平臺獨立觸發，互不影響',
    start: '開始備份',
    cancel: '取消備份',
    cancelling: '正在取消',
    cancelled: '備份已取消',
    cancelTitle: '取消目前備份？',
    cancelConfirm: '取消會立即停止檔案讀取或 Git 網路操作，並清理本次任務的臨時物件。已成功推送的資料不會回滾。',
    cancelRequested: '已送出取消請求',
    cancelError: '取消備份失敗',
    cancellingMsg: '正在停止目前備份操作…',
    info: '點擊「開始備份」將直接讀取備份來源並生成 Git 物件後推送至 {platform}；不會複製完整來源目錄，也不會檢出遠端備份檔案。',
    runningMsg: '任務進行中，請稍候…',
    jobMeta: '任務 #{id} · 節點 {server}',
    success: '備份成功',
    failed: '備份失敗',
    running: '備份進行中',
    unknown: '未知',
    startError: '啟動失敗',
    doneSuccess: '備份完成：成功',
    doneFailed: '備份完成：失敗',
    doneCancelled: '備份任務已取消',
    phasePreparing: '準備備份任務',
    phaseScanning: '掃描備份來源',
    phaseConnecting: '連接遠端倉庫',
    phaseFetching: '取得遠端提交與目錄樹中繼資料',
    phasePacking: '讀取來源檔案並生成 Git 物件',
    phaseCommitting: '建立備份提交',
    phasePushing: '推送至遠端倉庫',
    phaseRetrying: '等待重新同步後重試',
    phaseCancelling: '正在停止備份任務',
    notEnabled: '當前平臺未啟用，請先至「備份設定」中開啟',
    noToken: '尚未設定 Token，請先至「備份設定」中填寫'
  },
  schedule: {
    title: '定時任務',
    section: '{platform} 排程設定',
    enabled: '啟用定時備份',
    freq: '執行頻率',
    daily: '每天固定時間',
    hourly: '每隔 N 小時',
    custom: '自定義 Cron',
    dailyTime: '每日時間',
    pickTime: '選擇時間',
    dailyHint: '每天在該時刻執行一次備份',
    interval: '間隔(小時)',
    hourlyHint: '每隔 {n} 小時執行一次',
    cronExpr: 'Cron 表達式',
    cronPh: '分 時 日 月 周，例如 0 2 * * *',
    cronHint: '標準 5 段 Cron：分 時 日 月 周',
    next: '下次執行',
    lastRun: '上次自動執行',
    neverRun: '尚未執行',
    save: '儲存設定',
    test: '立即測試執行',
    on: '定時備份已開啟',
    onDesc: 'Cron: {cron}　|　伺服器端每 20 秒同步一次設定，無需重啟。',
    saved: '定時設定已儲存',
    loadError: '讀取設定失敗',
    triggered: '已觸發一次備份，任務 #{id}',
    triggerError: '觸發失敗',
    saveError: '儲存失敗',
    platformHint: '每個平臺獨立維護一份定時任務，切換上方 Tabs 即可分別設定。'
  },
  jobs: {
    title: '任務歷史',
    id: 'ID',
    platform: '平臺',
    server: '備份節點',
    status: '狀態',
    result: '結果',
    started: '開始時間',
    finished: '結束時間',
    actions: '操作',
    log: '日誌',
    logTitle: '備份日誌',
    noLog: '(無日誌)',
    empty: '暫無任務記錄',
    allPlatforms: '全部平臺'
  },
  users: {
    title: '用戶中心',
    add: '新增用戶',
    id: 'ID',
    username: '使用者名稱',
    role: '角色',
    admin: '管理員',
    viewer: '觀察者',
    created: '建立時間',
    actions: '操作',
    edit: '編輯',
    delete: '刪除',
    editTitle: '編輯用戶',
    createTitle: '新增用戶',
    usernamePh: '登入使用者名稱',
    password: '密碼',
    resetPwd: '重設密碼',
    pwdPh: '請輸入密碼',
    pwdResetPh: '留空則不修改',
    roleL: '角色',
    roleAdmin: '管理員',
    roleViewer: '觀察者',
    cancel: '取消',
    save: '儲存',
    saved: '已儲存',
    delConfirm: '確定刪除用戶「{user}」？',
    delTitle: '提示',
    deleted: '已刪除',
    saveError: '儲存失敗',
    delError: '刪除失敗',
    empty: '暫無用戶'
  },
  lang: {
    label: '語言',
    zhCN: '簡體中文',
    zhTW: '繁體中文',
    en: 'English'
  },
  responsive: {
    menu: '選單'
  }
}
