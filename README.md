# RepoArk
在服务器中运行服务，在WEB管理界面设置备份文件和定时任务，实现定时备份数据到GitHub、GitCode、Gitee私有仓库中去。
...
默认管理员账号：**admin / admin**（请尽快在「用户中心」中修改）。
...
Docker 部署时建议按下面的含义填写。临时任务目录和备份源必须分开：

```text
临时任务目录：/app/data/tmp/gitcode
备份节点名称：blog-prod
宿主机根路径映射：/host
备份源路径：/data/RepoArk
```

“备份节点名称”只是远程仓库中的一级目录名，用来区分不同 RepoArk 实例。首次启动会生成一个稳定名称并保存到 SQLite，也可以在页面中改成 `blog-prod`、`nas-home` 这类更易读的名称
